...
var environment = config['hdfc_sf-team_sf-demo_sf'].env;
...
console.log('&$hdfc_sf-team_sf-demo_sf api: [' +  domains[environment] + ', ' + config['hdfc_sf-team_sf-demo_sf'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hdfc_sf-team_sf-demo_sf'].app, config['hdfc_sf-team_sf-demo_sf'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hdfc_sf-team_sf-demo_sf'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
